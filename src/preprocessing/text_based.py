...
class TextBasedPreprocessor:
    ...
    def fit(self, smiles: list[str]) -> None:
        ...
        character_list = list(
            itertools.chain(*smiles)
        )  # Concat then split all characters: ["abc","ab"] -> ["a","b","c","a","b"]
        self.one_hot_encoder.fit(
            np.array(character_list).reshape(-1, 1)
        )  # Assign each unique character an index
        print(
            f"Done fitting one hot encoder, number of categories: {len(self.one_hot_encoder.get_feature_names_out())}."
        )
    ...
    def _transform_single(self, smiles: str) -> Data:
        ...
        x = self.one_hot_encoder.transform(
            np.array(list(smiles)).reshape(-1, 1)
        )  # Shape (num_characters, encoding_size)
        ...
        edge_pairs = [
            (a, b) for a, b in zip(node_indices[:-1], node_indices[1:])
        ]  # Forward edges
        edge_pairs.extend([(b, a) for a, b in edge_pairs])  # Backward edges
        edge_index = torch.tensor(
            edge_pairs
        ).T.contiguous()  # PyG format, see https://pytorch-geometric.readthedocs.io/en/2.5.1/get_started/introduction.html
